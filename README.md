...
```
themes/original/     # Fresh Start theme modules
├── base/            # Core interface colors
├── components/      # Buttons, inputs, scrollbars
├── editor/          # Editor-specific styling
├── panels/          # Sidebar, statusbar, tabs
├── syntax/          # Language-specific highlighting
├── terminal/        # Terminal colors
└── extensions/      # Git, debug, testing UI
...
Colors are defined in `theme-colors.json` and referenced using `{{variable-name}}` syntax throughout the theme. See `docs/COLOR_PALETTE.md` for details.
